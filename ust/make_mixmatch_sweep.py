...
# ───────────────────────────────
# Core sweep configuration for MixMatch
# ───────────────────────────────
BASE_SWEEP = {
    ...
    "parameters": {
        ...
        # === MixMatch specific hyperparameters ===
        "T_sharpen": {
...
for lbcl in LBCL_SIZES:
    for event in EVENTS:
        for set_num in SET_NUMS:
            ...
            # Naming convention: mixmatch_<event>_<lbcl>lbcl_set<set_num>
            sweep_cfg["name"] = f"mixmatch_{event}_{lbcl}lbcl_set{set_num}"
            sweep_cfg["description"] = (
                f"MixMatch HPO for {event} ({lbcl}lbcl) set{set_num}"
            )
...
with open("mixmatch_sweep_ids.txt", "w") as f:
    for sweep_id in ids:
        f.write(f"{sweep_id}\n")
print("✅ Saved sweep IDs to mixmatch_sweep_ids.txt")
